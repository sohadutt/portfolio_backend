...
class DeloitteJobsSpider(scrapy.Spider):
    ...
    def get_job_count(self, response):
        """
        Extracts the total number of job postings from the search results page.
        """
        count_text = response.css('span.searchResultsCount::text').get()
        if count_text:
            try:
                return int(count_text.replace(',', '').strip())
            except ValueError:
                self.logger.warning(f"Could not parse job count from text: {count_text}")
        return None
